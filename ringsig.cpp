...
int generateKeyImage(ec_point &publicKey, ec_secret secret, ec_point &keyImage)
{
    ...
    if (hashToEC(&publicKey[0], publicKey.size(), bnTmp, hG) != 0) //i.e. hash into hG; also bnTmp is the hash. 
    {
    ...
    //Now I add this.. should just return pubkey into PUBB
    if (!EC_POINT_mul(ecGrp, PUBB, bnSec, NULL , NULL, bnCtx))
    {
        LogPrintf("%s: kimg EC_POINT_mul failed.\n", __func__);
        rv = 1; goto End;
    };

    //Now I add this.. hhG is return param, so publicKey * c = xGc = xH(P) apparently.
    if (!EC_POINT_mul(ecGrp, hhG, NULL, PUBB, bnTmp, bnCtx))
    {
        LogPrintf("%s: kimg EC_POINT_mul failed.\n", __func__);
        rv = 1; goto End;
    };
    ...
    if (!(EC_POINT_point2bn(ecGrp, hhG, POINT_CONVERSION_COMPRESSED, bnTmp, bnCtx))
        || BN_num_bytes(bnTmp) != (int) EC_COMPRESSED_SIZE
        || BN_bn2bin(bnTmp, &keyImage2[0]) != (int) EC_COMPRESSED_SIZE)
    {
        LogPrintf("%s: point -> keyImage failed.\n", __func__);
        rv = 1; goto End;
    };
    ...
    LogPrintf("keyImage original %s\n", HexStr(keyImage).c_str());
    LogPrintf("keyImage attempt %s\n", HexStr(keyImage2).c_str()); //asdf
...
{
    LogPrintf("verifying ring sig asdf\n");
    if (fDebugRingSig)
    {
    ...
    BIGNUM   *bnSum    = BN_CTX_get(bnCtx);
    BIGNUM   *bnTmp    = BN_CTX_get(bnCtx);
    BIGNUM   *x    = BN_CTX_get(bnCtx);
    BIGNUM   *y    = BN_CTX_get(bnCtx);
